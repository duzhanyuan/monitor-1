...
long long start_tsc;            /* Timestamp counter at start. */
...
thread_print_stats (void) 
{
  printf ("Thread: %lld idle ticks, %lld kernel ticks, %lld user ticks, "
      "%lld tsc\n", idle_ticks, kernel_ticks, user_ticks, rdtsc() - start_tsc);
}
               ...
               thread_func *function, void *aux) 
{
  ...
  /* Initialize thread. */
  init_thread (t, name, thread_mlfqs ? cur->priority : priority,allocate_tid());
  tid = t->tid;
  t->nice = cur->nice;
