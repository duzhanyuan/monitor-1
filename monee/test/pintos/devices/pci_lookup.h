...
struct pci_class pci_class_table[] = {
  ...
  {6, 4, 0, "PCI-PCI Bridge (normal decode)"},
  {6, 4, 1, "PCI-PCI Bridge (subtractive decode)"},
...
pci_lookup_class (uint8_t major, uint8_t minor, uint8_t iface)
{
  ...
  snprintf(ret, sizeof ret, "Unknown Type: 0x%hhx,0x%hhx,0x%hhx", major, minor,
      iface);
  return ret;
}
