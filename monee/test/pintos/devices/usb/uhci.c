...
#define ptov_uhci(x) ((((uint32_t)(x))&UHCI_PTR_T)?({ASSERT((uint32_t)(x)==UHCI_PTR_T); (void *)(x);}):ptov(x))
#define vtop_uhci(x) vtop(x)
...
uhci_alloc_td(struct uhci_data *uhci UNUSED)
{
  struct td_node *n;
 ...
 */
static void
uhci_free_td_quick(struct uhci_data *uhci UNUSED, struct uhci_transfer_desc *td)
{
  //struct td_node *t;
  enum intr_level old_level;
  struct list_elem *e;
...
void uhci_free_td(struct uhci_data *uhci UNUSED,
		struct uhci_transfer_desc *td)
{
...
static void
uhci_insert_interrupt(struct uhci_data *uhci UNUSED,
		struct usb_pipe *pipe UNUSED)
{
  NOT_IMPLEMENTED();
#if 0
...
struct usb_pipe *
uhci_usbdevice_create_pipe(struct usbdevice *dev UNUSED, enum usb_pipetype type,
    uint8_t endpoint, uint8_t period UNUSED, uint32_t maxpacket UNUSED,
