...
void
usbhub_port_enable(struct usbhub *hub UNUSED)
{
}
...
usbhub_explore(struct usbhub *hub)
{
  ...
  for (port = 1; port <= hub->descriptor.bNbrPorts; port++) {
    ...
    if (!usbhub_get_port_status(hub, port, &port_status)) {
      printf("pintos: %s(): usbhub_get_port_status(%p, %d, %p) failed.\n",
					__func__, hub, port, &status);
      continue;
    }
    ...
    if (!(status & UPS_CURRENT_CONNECT_STATUS)) {
      printf("pintos: %s(): !CURRENT_CONNECT_STATUS\n", __func__);
      continue;
    }
    ...
    if (!usbhub_port_reset(hub, port)) {
      DBGn(USB, "%s(): port %d reset failed\n", __func__, port);
      continue;
    }
    if (!usbhub_get_port_status(hub, port, &port_status)) {
      DBGn(USB, "%s(): usbhub_get_port_status(%p, %d, %p) failed\n", __func__,
          hub, port, &status);
      continue;
    }
    status = port_status.wPortStatus;
    ...
    if (!(status & UPS_CURRENT_CONNECT_STATUS)) {
      DBGn(USB, "%s(): device on port %d disappeared after reset???\n",
          __func__, port);
      continue;
    }
