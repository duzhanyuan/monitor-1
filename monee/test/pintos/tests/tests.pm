...
die if @ARGV != 4;
our ($test, $src_dir, $SRCDIR, $variant_dir) = @ARGV;
...
sub check_for_panic {
	...
	my $trace;
	if ($variant_dir eq "pintos") {
		$trace = scalar (`$SRCDIR/misc/backtrace kernel.o $userprog $addrs`);
	} else {
		$trace = scalar (`$SRCDIR/misc/backtrace $variant_dir/mon/monitor.o \
			$userprog $addrs`);
	}
...
sub finish {
    ...
    if ($verdict eq 'PASS') {
			my ($output_fn) = "$test.output";
			my ($total_ticks, $idle_ticks, $kernel_ticks, $user_ticks, $guest_tsc);
			my ($monitor_ticks, $guest_ticks, $monitor_tsc);
			my ($tb_replacements);
			my ($tb_size_avg, $tb_size_min, $tb_size_max, $tb_max_pages_allowed);
			my ($tb_page_count);
			my ($swap_pd_supervisor_replacements);
			my ($swap_pd_user_replacements);
			my ($swap_pt_supervisor_replacements);
			my ($swap_pt_user_replacements);
			my ($swap_pg_replacements);
			my ($swap_replacements);
			my ($swap_pd_supervisor_size_avg, $swap_pd_supervisor_size_min, $swap_pd_supervisor_size_max);
			my ($swap_pd_user_size_avg, $swap_pd_user_size_min, $swap_pd_user_size_max);
			my ($swap_pt_supervisor_size_avg, $swap_pt_supervisor_size_min, $swap_pt_supervisor_size_max);
			my ($swap_pt_user_size_avg, $swap_pt_user_size_min, $swap_pt_user_size_max);
			my ($swap_pg_size_avg, $swap_pg_size_min, $swap_pg_size_max);
			my ($total_page_faults, $true_page_faults, $mtraced_page_faults);
			my ($shadow_page_faults, $phys_map_page_faults);
			my ($all_callouts, $forced_callouts);
			my $mstats = "MON-STATS";
			open (OUTPUT, '<', $output_fn) or die "$output_fn: open: $!\n";
			while (my $line = <OUTPUT>) {
				if ($line =~ /Timer: (\d*) ticks/) {
					$total_ticks = $1;
				} elsif ($line =~ /(\d+) idle ticks, (\d+) kernel ticks, (\d+) user ticks, (\d+) tsc/) {
					$idle_ticks = $1;
					$kernel_ticks = $2;
					$user_ticks = $3;
          $guest_tsc = $4;
				} elsif ($line =~
					/^$mstats: Thread: (\d*) monitor ticks, (\d*) guest ticks, (\d+) tsc/) {
					$monitor_ticks = $1;
					$guest_ticks = $2;
          $monitor_tsc = $3;
				} elsif ($line =~ /^$mstats: (\d*) translation block replacements\./) {
					$tb_replacements = $1;
				} elsif ($line =~ /^$mstats: Size of translation cache: \[(\d*) avg, (\d*) min, (\d*) max\] \((\d*)\/(\d*) pages\)/) {
					$tb_size_avg = $1;
					$tb_size_min = $2;
					$tb_size_max = $3;
					$tb_page_count = $4;
					$tb_max_pages_allowed = $5;
				} elsif ($line =~ /^$mstats: swap: (\d*) replacements \((\d*),(\d*),(\d*),(\d*),(\d*)\)\./) {
					$swap_replacements = $1;
					$swap_pd_supervisor_replacements = $2;
					$swap_pd_user_replacements = $3;
					$swap_pt_supervisor_replacements = $4;
					$swap_pt_user_replacements = $5;
					$swap_pg_replacements = $6;
				} elsif ($line =~ /^$mstats: swap pd-supervisor: \[(\d*) avg, (\d*) min, (\d*) max\]/) {
					$swap_pd_supervisor_size_avg = $1;
					$swap_pd_supervisor_size_min = $2;
					$swap_pd_supervisor_size_max = $3;
				} elsif ($line =~ /^$mstats: swap pd-user: \[(\d*) avg, (\d*) min, (\d*) max\]/) {
					$swap_pd_user_size_avg = $1;
					$swap_pd_user_size_min = $2;
					$swap_pd_user_size_max = $3;
				} elsif ($line =~ /^$mstats: swap pt-supervisor: \[(\d*) avg, (\d*) min, (\d*) max\]/) {
					$swap_pt_supervisor_size_avg = $1;
					$swap_pt_supervisor_size_min = $2;
					$swap_pt_supervisor_size_max = $3;
				} elsif ($line =~ /^$mstats: swap pt-user: \[(\d*) avg, (\d*) min, (\d*) max\]/) {
					$swap_pt_user_size_avg = $1;
					$swap_pt_user_size_min = $2;
					$swap_pt_user_size_max = $3;
				} elsif ($line =~ /^$mstats: swap pg: \[(\d*) avg, (\d*) min, (\d*) max\]/) {
					$swap_pg_size_avg = $1;
					$swap_pg_size_min = $2;
					$swap_pg_size_max = $3;
				} elsif ($line =~ /^$mstats: exception: (\d*) page-faults \((\d*) t, (\d*) m, (\d*) s, (\d*) p\)/) {
					$total_page_faults = $1;
					$true_page_faults = $2;
					$mtraced_page_faults = $3;
					$shadow_page_faults = $4;
					$phys_map_page_faults = $5;
				} elsif ($line =~ /^$mstats: callouts: (\d*) all, (\d*) forced/) {
					$all_callouts = $1;
					$forced_callouts = $2;
				}
			}
			close(OUTPUT);
			my $testcore = $test;
			$testcore =~ s/tests\/threads\///;
			$testcore =~ s/pintos$//;
			my $outstr = "$testcore";
			my $pos = 40;
			$outstr .= indent($pos - length $outstr);
			print STDOUT "$outstr [$total_ticks ticks ";
			print STDOUT "($kernel_ticks k, ";
			print STDOUT "$user_ticks u), $guest_tsc tsc]\n";

			if (defined $monitor_ticks) {
				$outstr = indent($pos);
				print STDOUT "$outstr [$monitor_ticks m, ";
				print STDOUT "$guest_ticks g, $monitor_tsc tsc]\n";

				$outstr = indent($pos);
				print STDOUT "$outstr [tb: $tb_replacements: ";
				print STDOUT "$tb_size_avg, $tb_size_min, $tb_size_max: ";
				print STDOUT "$tb_page_count/$tb_max_pages_allowed]\n";

				$outstr = indent($pos);
				print STDOUT "$outstr [swap: pd-s: $swap_pd_supervisor_replacements: ";
				print STDOUT "$swap_pd_supervisor_size_avg, ";
				print STDOUT "$swap_pd_supervisor_size_min, ";
				print STDOUT "$swap_pd_supervisor_size_max]\n";

				$outstr = indent($pos);
				print STDOUT "$outstr [swap: pd-u: $swap_pd_user_replacements: ";
				print STDOUT "$swap_pd_user_size_avg, ";
				print STDOUT "$swap_pd_user_size_min, ";
				print STDOUT "$swap_pd_user_size_max]\n";

				$outstr = indent($pos);
				print STDOUT "$outstr [swap: pt-s: $swap_pt_supervisor_replacements: ";
				print STDOUT "$swap_pt_supervisor_size_avg, ";
				print STDOUT "$swap_pt_supervisor_size_min, ";
				print STDOUT "$swap_pt_supervisor_size_max]\n";

				$outstr = indent($pos);
				print STDOUT "$outstr [swap: pt-u: $swap_pt_user_replacements: ";
				print STDOUT "$swap_pt_user_size_avg, ";
				print STDOUT "$swap_pt_user_size_min, ";
				print STDOUT "$swap_pt_user_size_max]\n";

				$outstr = indent($pos);
				print STDOUT "$outstr [swap: pg: $swap_pg_replacements: ";
				print STDOUT "$swap_pg_size_avg, ";
				print STDOUT "$swap_pg_size_min, ";
				print STDOUT "$swap_pg_size_max]\n";

				$outstr = indent($pos);
				print STDOUT "$outstr [page-faults: $total_page_faults: ";
				print STDOUT "$true_page_faults t, ";
				print STDOUT "$mtraced_page_faults m, ";
				print STDOUT "$shadow_page_faults s, ";
				print STDOUT "$phys_map_page_faults p]\n";

				$outstr = indent($pos);
				print STDOUT "$outstr [callouts: $all_callouts all, ";
				print STDOUT "$forced_callouts forced]\n";
			}
    } else {
			print STDERR "FAIL $test\n";
    }
    print STDERR "$_\n" foreach @messages;
