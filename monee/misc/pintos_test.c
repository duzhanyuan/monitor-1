...
producer_thread(void *aux)
{
  ...
  for (i = 0; i < prodcon_struct->num_elements; i++) {
    sema_down(holes);
    lock_acquire(queue_head_lock);//XXX
    ...
    lock_release(queue_head_lock);//XXX
    sema_up(chars);
  }
...
consumer_thread(void *aux)
{
  ...
  while (1) {
    ...
    //printf("%s() %d:\n", __func__, __LINE__);
    sema_down(chars);
    lock_acquire(queue_tail_lock);//XXX
    ...
    lock_release(queue_tail_lock);//XXX
    sema_up(holes);
...
producer_consumer_test(void)
{
  ...
  printf("%s():\n", __func__);
  num_elements = 1000;
  producer_consumer(200, 200, num_elements, 10, 1000);
  producer_consumer(400, 400, num_elements, 2, 1000);
#if 0
...
producer_consumer_no_locks_test(void)
{
  ...
  num_elements = 1000000;
	producer_consumer_no_locks(num_elements, 20, 1000);
	/*
...
struct updater_struct {
  int64_t n_iter;
  struct semaphore *done_sema;
};

static long long debit, credit, total;

static void
updater_thread(void *aux)
{
	struct updater_struct *updater_struct = aux;
#define idle() do {		\
	int ii;							\
	for (ii = 0; ii < 1; ii++); \
} while (0)
...
#define transfer() do {																										\
		...
		credit = lcredit;                                                     \
		idle();                                                               \
		ASSERT(debit + credit == total);                                      \
} while (0)
	long long i;

	for (i = 0; i < updater_struct->n_iter; i++) {
#if CRITICAL_SECTION_LENGTH==1
		transfer();
#elif CRITICAL_SECTION_LENGTH==2
		transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==3
		transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==4
		transfer(); transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==5
		transfer(); transfer(); transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==6
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==7
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer();
#elif CRITICAL_SECTION_LENGTH==8
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==9
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==10
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==11
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==12
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==13
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer();
#elif CRITICAL_SECTION_LENGTH==14
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==15
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==16
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==17
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer(); transfer();
#elif CRITICAL_SECTION_LENGTH==18
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
		transfer(); transfer(); transfer(); transfer(); transfer(); transfer();
#else
	...
	printf("debit=%llx, credit=%llx, sum=%llx, total=%llx\n", debit, credit,
			debit + credit, total);
	sema_up(updater_struct->done_sema);
	printf("done\n");
}

static void
concurrent_updates(int t, int n)
{
	struct updater_struct *updater_struct;
	struct semaphore done_sema;
  tid_t *updaters;
	int i;

	updaters = malloc(sizeof(tid_t)*t);
	ASSERT(updaters);
	updater_struct = malloc(sizeof(struct updater_struct)*t);
	ASSERT(updater_struct);
	...
	for (i = 0; i < t; i++) {
		...
		updater_struct[i].n_iter = n/10*t;
		updater_struct[i].done_sema = &done_sema;
		updaters[i] = thread_create(name, PRI_DEFAULT, updater_thread,
				&updater_struct[i]);
		ASSERT(updaters[i] != TID_ERROR);
	}

	for (i = 0; i < t; i++) {
		sema_down(&done_sema);
	}
	printf("%s() %d: debit=%llx, credit=%llx\n", __func__, __LINE__,
			debit, credit);
	free(updaters);
	free(updater_struct);
}
...
static void
concurrent_updates_test(void)
{
	int64_t start;
	printf("%s() entry:\n", __func__);
	start = timer_ticks();
	concurrent_updates(2, 400000);
	printf("%s(): elapsed=%lld\n", __func__, timer_elapsed(start));
	printf("%s(): Printing statistics:\n", __func__);
	print_stats();
}
