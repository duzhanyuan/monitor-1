...
static inline void
outb (uint16_t port, uint32_t data)
{
  /* See [IA32-v2b] "OUT". */
  asm volatile ("outb %0,%w1" : : "a" ((uint8_t)data), "d" (port));
}
...
static inline void
outw (uint16_t port, uint32_t data)
{
  /* See [IA32-v2b] "OUT". */
  asm volatile ("outw %0,%w1" : : "a" ((uint16_t)data), "d" (port));
}
...
int register_ioport_read(int start, int length, int size, IOPortReadFunc *func,
    void *opaque, bool log);
int register_ioport_write(int start, int length, int size,
    IOPortWriteFunc *func, void *opaque, bool log);
bool ioport_needs_log(uint16_t port);

/* Handler functions for I/O. */
void io_out(uint16_t port, target_ulong data, size_t data_size);
void io_outs(uint16_t port, const void *addr, size_t cnt, size_t data_size);
void io_ins(uint16_t port, void *addr, size_t cnt, size_t data_size);

#define io_in(suffix, type) \
