...
static int pci_probe (int bus, int dev, int func,
		      struct pci_config_header *ph, int max_bus);
static int pci_pci_bridge (struct pci_dev *pd, int cur_max_bus);
...
static void *pci_alloc_mem (target_phys_addr_t phys_ptr, int pages);
...
#if 0
static void
pci_alloc_mem(struct pci_dev *pd)
{
  struct list_elem *e;
  for (e = list_begin(&pd->io_ranges); e != list_end(&pd->io_ranges);
      e = list_next(e)) {
    struct pci_io *pio;
    pio = list_entry(e, struct pci_io, peer);
    if (pio->type == PCI_IO_MEM
        && ((target_phys_addr_t)pio->addr.ptr & PCI_MEM_ADDR_INVALID)) {
      target_phys_addr_t paddr;
      int i;
      if (pio->size > PGSIZE) {
        NOT_IMPLEMENTED();
      }
      ASSERT(pio->size == PGSIZE);
      for (i = 0; i < PCI_BASE_COUNT; i++) {
        uint32_t tmp;
        tmp = pci_read_config(pd->bus, pd->dev, pd->func,
            offsetof(struct pci_config_header, pci_base_reg[i]), 4);
        if (tmp == ((target_phys_addr_t)pio->addr.ptr & ~PCI_MEM_ADDR_INVALID)){
          break;
        }
      }
      ASSERT(i < PCI_BASE_COUNT);
      pio->addr.ptr = palloc_get_page(PAL_ASSERT | PAL_ZERO);
#ifdef __MONITOR__
      paddr = vtop_mon(pio->addr.ptr);
#else
      paddr = (target_phys_addr_t)pio->addr.ptr;
#endif
      /* configure BAR to the default */
      pci_write_config (pd->bus, pd->dev, pd->func,
          offsetof (struct pci_config_header, pci_base_reg[i]), 4, paddr);
      ASSERT(pci_read_config(pd->bus, pd->dev, pd->func,
            offsetof(struct pci_config_header, pci_base_reg[i]), 4) == paddr);
      pd->pch.pci_base_reg[i] = paddr;
    }
  }
}
#endif
...
target_phys_addr_t
pci_io_base (struct pci_io *pio)
{
  ASSERT (pio != NULL);

  switch (pio->type) {
    case PCI_IO_PORT:
      return pio->addr.port;
    case PCI_IO_MEM:
      return (target_phys_addr_t)pio->addr.ptr;
		default:
			NOT_REACHED();
  }
}
...
pci_reg_write32 (struct pci_io *pio, int reg, uint32_t data)
{
  ...
  if (pio->type == PCI_IO_MEM) {
    ...
    *((uint32_t *) ((uint8_t *)pio->addr.ptr + reg)) = data;
    switch_pt(pt_mode);
...
pci_reg_write16 (struct pci_io *pio, int reg, uint16_t data)
{
  ...
  if (pio->type == PCI_IO_MEM) {
    pt_mode_t pt_mode;
    pt_mode = switch_to_phys();
    *((uint16_t *) ((uint8_t *)pio->addr.ptr + reg)) = data;
    switch_pt(pt_mode);
  } else if (pio->type == PCI_IO_PORT) {
...
pci_reg_write8 (struct pci_io *pio, int reg, uint8_t data)
{
  ...
  if (pio->type == PCI_IO_MEM) {
    pt_mode_t pt_mode;
    pt_mode = switch_to_phys();
    *((uint8_t *) ((uint8_t *)pio->addr.ptr + reg)) = data;
    switch_pt(pt_mode);
  } else if (pio->type == PCI_IO_PORT) {
...
pci_reg_read32 (struct pci_io *pio, int reg)
{
  ...
  if (pio->type == PCI_IO_MEM) {
    pt_mode_t pt_mode;
    pt_mode = switch_to_phys();
    ret = *((uint32_t *) ((uint8_t *)pio->addr.ptr + reg));
    switch_pt(pt_mode);
  } else if (pio->type == PCI_IO_PORT) {
...
pci_reg_read16 (struct pci_io * pio, int reg)
{
  ...
  if (pio->type == PCI_IO_MEM) {
    pt_mode_t pt_mode;
    pt_mode = switch_to_phys();
    ret = *((uint16_t *) ((uint8_t *)pio->addr.ptr + reg));
    switch_pt(pt_mode);
  } else if (pio->type == PCI_IO_PORT) {
...
pci_reg_read8 (struct pci_io * pio, int reg)
{
  ...
  if (pio->type == PCI_IO_MEM) {
    pt_mode_t pt_mode;
    pt_mode = switch_to_phys();
    printf("%s() %d: pio->addr.ptr=%p, reg=%d\n", __func__, __LINE__, pio->addr.ptr, reg);
    ret = *((uint8_t *) ((uint8_t *)pio->addr.ptr + reg));
    switch_pt(pt_mode);
  } else if (pio->type == PCI_IO_PORT) {
...
pci_scan_bus (int bus)
{
  ...
  for (dev = 0; dev < PCI_MAX_DEV_PER_BUS; dev++) {
    ...
    for (func = 0; func < func_cnt; func++) {
      int retbus;
      retbus = pci_probe (bus, dev, func, &pch, max_bus);
      ASSERT(retbus >= max_bus);
      max_bus = retbus;
    }
  }
  return max_bus;
    ...
    int max_bus)
{
  ...
  if (ph->pci_major == PCI_MAJOR_BRIDGE) {
    if (ph->pci_minor == PCI_MINOR_PCI)
      return pci_pci_bridge (pd, max_bus);
  }
...
pci_setup_io (struct pci_dev *pd)
{
  ...
  for (i = 0; i < PCI_BASE_COUNT; i++) {
    ...
    if (tmp & PCI_BASEADDR_IO) {
      ...
      pio->addr.ptr = (void *)(pd->pch.pci_base_reg[i] & 0xfffffff0);  /* sorav */
      /*
      pio->addr.ptr = pci_alloc_mem (pd->pch.pci_base_reg[i],
          pio->size / PGSIZE);
...
static int
pci_pci_bridge (struct pci_dev *pd, int cur_max_bus)
{
  int max_bus;
  uint16_t command;
  ...
  /* secondary bus */
  pci_write_config (pd->bus, pd->dev, pd->func, 0x19, 1, cur_max_bus + 1);

  /* disable subordinates */
  pci_write_config (pd->bus, pd->dev, pd->func, 0x1a, 1, 0xff);

  /* scan this new bus */
  max_bus = pci_scan_bus (cur_max_bus + 1);
...
{
  struct list_elem *e;
  MSG ("PCI Device %d:%d.%d (%x,%x): %s - %s (%s) IRQ %d\n",
  ...
  for (e = list_begin(&pd->io_ranges); e != list_end(&pd->io_ranges);
      e = list_next(e)) {
    struct pci_io *io;
    io = list_entry(e, struct pci_io, peer);
    if (io->type == PCI_IO_PORT) {
      MSG ("0x%x-0x%x ", io->addr.port, io->addr.port + io->size);
    }
  }
  MSG ("]\n");
  MSG ("\tmem: [");
  for (e = list_begin(&pd->io_ranges); e != list_end(&pd->io_ranges);
    ...
    struct pci_io *io;
    io = list_entry(e, struct pci_io, peer);
    if (io->type == PCI_IO_MEM) {
      MSG ("%p-%p ", io->addr.ptr, (uint8_t *)io->addr.ptr + io->size);
    }
  }
  MSG ("]\n");
}
...
struct pci_dev *
pio_to_pci_dev (struct pci_io *pio)
{
  return pio->dev;
}
...
static void *
pci_alloc_mem (target_phys_addr_t phys_ptr, int pages)
{
  void *vaddr;
  int i;
  phys_ptr = (uintptr_t) phys_ptr & ~PGMASK;
