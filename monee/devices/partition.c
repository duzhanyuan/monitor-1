...
partition_scan (struct block *block)
{
  ...
  if (part_nr == 0) {
    MSG ("%s: Device contains no partitions\n", block_name (block));
	}
}
                      ...
                      int *part_nr)
{
  ...
  if (sector >= block_size (block))
    {
      MSG ("%s: Partition table at sector %"PRDSNu" past end of device.\n",
  ...
  if (pt->signature != 0xaa55)
    {
			if (primary_extended_sector == 0) {
				MSG ("%s: Invalid partition table signature\n", block_name (block));
			} else {
        MSG ("%s: Invalid extended partition table in sector %"PRDSNu"\n",
                block_name (block), sector);
			}
      free (pt);
      return;
  ...
  for (i = 0; i < sizeof pt->partitions / sizeof *pt->partitions; i++)
    {
      ...
      if (e->size == 0 || e->type == 0)
        ...
        {
          MSG ("%s: Extended partition in sector %"PRDSNu"\n",
                  block_name (block), sector);
                 ...
                 int part_nr)
{
  if (start >= block_size (block)) {
    MSG ("%s%d: Partition starts past end of device (sector %"PRDSNu")\n",
        block_name (block), part_nr, start);
  } else if (start + size < start || start + size > block_size (block)) {
    MSG ("%s%d: Partition end (%"PRDSNu") past end of device (%"PRDSNu")\n",
    ...
    enum block_type type = (part_type == 0x20 ? BLOCK_KERNEL
        ...
        : BLOCK_FOREIGN);
        */
    enum block_type type = BLOCK_RAW;
    struct partition *p;
    char extra_info[128];
    char name[16];
