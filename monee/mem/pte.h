...
#define LPTE_ADDR 0xffc00000    /* Address bits for large page. */

/* Returns a PDE that points to page table PT. */
static inline uint32_t pde_create (uint32_t *pt, bool writable) {
  ASSERT (pg_ofs(pt) == 0);
  return vtop_mon(pt) | PTE_U | PTE_P | (writable ? PTE_W : 0);
}

/* Returns a pointer to the page table that page directory entry
   PDE, which must "present", points to. */
static inline uint32_t *pde_get_pt_mon (uint32_t pde) {
  return ptov_mon (pde & PTE_ADDR);
}
...
static inline uint32_t pte_create (uintptr_t paddr, bool writable) {
  ASSERT (pg_ofs (ptov_phy(paddr)) == 0);
  return (paddr) | PTE_U | PTE_P | (writable ? PTE_W : 0);
}

/* Returns a pointer to the page that page table entry PTE points
   to. */
static inline void *pte_get_page (uint32_t pte) {
  return ptov_phy (pte & PTE_ADDR);
}
...
/* PDEs
   ...
   31                                          13 12            0
   +----------------------------+----------------+---------------+
   |     Page Base Address      |   Reserved     |       Flags   |
   +----------------------------+----------------+---------------+

   The page size bit (bit 7) must be set to indicate 4MB pages.
   A PDE that is initialized to 0 will be interpreted as
   "not present", which is just fine. */

#define PTE_PS 0x80             /* Page size. 1=4MB, 0=4KB. */
#define PTE_G  0x100            /* 1=Global. preserves TLB entry on switch. */

/* Returns a PDE that points to page table PT. */
static inline uint32_t pde_create_large (uintptr_t paddr, bool writable) {
  ASSERT (pg_ofs (ptov_phy(paddr)) == 0);
  return (paddr) | PTE_U | PTE_PS | PTE_G | PTE_P | (writable ? PTE_W : 0);
}

/* Returns a monitor PDE that points to page table PT. */
static inline uint32_t pde_create_mon (uintptr_t paddr, bool writable) {
  return (paddr) | PTE_U | PTE_P | (writable ? PTE_W : 0);
}

/* Returns a monitor PDE that points to large page PADDR. */
static inline uint32_t pde_create_large_mon (uintptr_t paddr, bool writable) {
  return (paddr) | PTE_U | PTE_PS | PTE_G | PTE_P | (writable ? PTE_W : 0);
}
