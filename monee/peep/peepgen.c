...
output_code_with_rollback(FILE *out_S, char const *label, char const *out_text,
    char const *rollback_text, assignments_t const *assignments,
    cpu_mode_t mode)
{
  char rollback_label[strlen(label) + 64];
  snprintf(rollback_label, sizeof rollback_label, ROLLBACK_PREFIX_STR "%s",
      label);
  rename_and_output_as_code(out_S, label, out_text, assignments, mode, true);

  rbcode_ptr += snprintf(rbcode_ptr, rbcode_end - rbcode_ptr,
      "\n.global %s\n", rollback_label);
  rbcode_ptr += snprintf(rbcode_ptr, rbcode_end - rbcode_ptr,
      "%s:\n", rollback_label);
  rbcode_ptr += snprintf(rbcode_ptr, rbcode_end - rbcode_ptr,
      ".code32\n");
  rename_as_code(rbcode_ptr, rbcode_end - rbcode_ptr, rollback_text,
      assignments, false);
  rbcode_ptr += strlen(rbcode_ptr);
  //printf("%s:rollback_text=%s\n", label, rollback_text);
  ASSERT(rbcode_ptr < rbcode_end);
  /* rename_and_output_as_code(out_S, rollback_label, rollback_text,
      assignments, mode, false); */
}

static void
generate_output_code(FILE *out_S, char const *index_name,
    char const *out_text, char const *rollback_text,
    assignments_t const *assignments)
{
  ...
  char renamed_rollback_text[strlen(rollback_text) + 64];
  ...
  output_code_with_rollback(out_S, label, out_text, rollback_text, assignments,
      cpu_mode_32);
  ...
  for (i = 0; i < num_regs; i++) {
    ...
    for (j = 0; j < NUM_REGS; j++) {
      ...
      if (assignment_is_coherent(&rank_assignments.arr[r].var,
        ...
        make_string_replacements(renamed_rollback_text,
            sizeof renamed_rollback_text, rollback_text, patterns,
            replacements, 1);
        output_code_with_rollback(out_S, label, renamed_out_text,
            renamed_rollback_text, &rank_assignments, cpu_mode_32);
      }
...
static size_t
find_rollback_code(char *rollback_text, size_t rollback_text_size,
    char const *out_text, char const *label)
{
#define MAX_RB_LINES 128
#define EXCP_PATTERN "excp"
  char const *ptr = out_text;
  char *optr, *oend;
  char *rb_lines[MAX_RB_LINES];
  int nb_rb_lines = 0, i;
  while ((ptr = strstr(ptr, EXCP_PATTERN)) != NULL) {
    char const *hashchr, *newline, *semicolon, *end_line, *eptr;
    eptr = ptr + strlen(EXCP_PATTERN);
    hashchr = strchr(eptr, '#');
    ASSERT(hashchr);
    hashchr++;
    newline = strchr(eptr, '\n');
    ASSERT(newline);
    semicolon = strchr(eptr, ';');
    if (semicolon && semicolon < newline) {
      end_line = semicolon;
    } else {
      end_line = newline;
    }
    rb_lines[nb_rb_lines] = malloc(end_line - hashchr + 2);
    ASSERT(rb_lines[nb_rb_lines]);
    memcpy(rb_lines[nb_rb_lines], hashchr, end_line - hashchr + 1);
    rb_lines[nb_rb_lines][end_line - hashchr + 1] = '\0';
    ptr = hashchr;
    nb_rb_lines++;
  }
  optr = rollback_text;
  oend = rollback_text + rollback_text_size;
  for (i = nb_rb_lines - 1; i >= 0; i--) {
    ASSERT(i >= 0);
    optr += snprintf(optr, oend - optr, ROLLBACK_PREFIX_STR EXCP_PATTERN
        "%02d_%s: %s", i, label, rb_lines[i]);
    free(rb_lines[i]);
    ASSERT(optr < oend);
  }
  *optr = '\0';
  /* printf("%s(): returning \"%s\" for \"%s\"\n", __func__, rollback_text,
      out_text); */
  return optr - rollback_text;
}
    ...
    FILE *out_S, FILE *vars_fp, FILE *constraints_fp/*, FILE *used_regs_fp*/)
{
  ...
  find_rollback_code(rollback_text, sizeof rollback_text, out_text, index_name);
  /* Only those variables must be assigned before generating output code,
   * which do not emit relocations. */
  generate_output_code(out_S, index_name, out_text,
      rollback_text, &assignments);
  ...
  //output_used_regs(used_regs_fp, index_name, &used_regs);
}
...
#if 0
  snprintf(lmem_constraints[0], sizeof lmem_constraints[0],
      "used_regs : ebx,esi\n");
  snprintf(lmem_constraints[1], sizeof lmem_constraints[1],
      "used_regs : ebx,edi\n");
  snprintf(lmem_constraints[2], sizeof lmem_constraints[2],
      "used_regs : ebp,esi\n");
  snprintf(lmem_constraints[3], sizeof lmem_constraints[3],
      "used_regs : ebp,edi\n");
  snprintf(lmem_constraints[4], sizeof lmem_constraints[4],
      "used_regs : esi\n");
  snprintf(lmem_constraints[5], sizeof lmem_constraints[5],
      "used_regs : edi\n");
  snprintf(lmem_constraints[6], sizeof lmem_constraints[6],
      "used_regs : ebp\n");
  snprintf(lmem_constraints[7], sizeof lmem_constraints[7],
      "used_regs : ebx\n");
  lmem_constraints[8][0] = '\0';  /* by default, no additional constraints. */
#endif
...
gen_as_files(char const *filename)
{
  ...
  //used_regs_fp = xfopen("used_regs", "w");
  ...
  while (fscanf(fp, "%[^\n]\n", line) != EOF) {
    ...
    for (i = 0; i < num_mem_modes; i++) {
      ...
      for (j = 0; j < num_jcc_conds; j++) {
        ...
        int num_old_patterns = num_patterns, p, k;
...
gen_offsets(void)
{
  ...
  fprintf(fp, "#define VCPU_CR_OFF(i) (%d+i*%d)\n",
      offsetof(vcpu_t, cr[0]), sizeof vcpu.cr[0]);
  ...
  fprintf(fp, "#define VCPU_IF_OFF %d\n", offsetof(vcpu_t, IF));
	/*
  fprintf(fp, "#define VCPU_STI_FALLTHROUGH_OFF %d\n",
      offsetof(vcpu_t, sti_fallthrough));
			*/
  fprintf(fp, "#define VCPU_IOPL_OFF %d\n", offsetof(vcpu_t, IOPL));
  fprintf(fp, "#define VCPU_AC_OFF %d\n", offsetof(vcpu_t, AC));
  fprintf(fp, "#define VCPU_EIP_OFF %d\n", offsetof(vcpu_t, eip));
  fprintf(fp, "#define VCPU_EIP_EXECUTING_OFF %d\n",
			offsetof(vcpu_t, eip_executing));
  fprintf(fp, "#define VCPU_NEXT_EIP_IS_SET_OFF %d\n",
      offsetof(vcpu_t, next_eip_is_set));
  fprintf(fp, "#define VCPU_INTERRUPT_REQUEST_OFF %d\n",
      offsetof(vcpu_t, interrupt_request));
  fprintf(fp, "#define VCPU_REGS_OFF(i) (%d+i*%d)\n",
      offsetof(vcpu_t, regs[0]), sizeof vcpu.regs[0]);
  fprintf(fp, "#define VCPU_EAX_OFF VCPU_REGS_OFF(0)\n");
  fprintf(fp, "#define VCPU_ECX_OFF VCPU_REGS_OFF(1)\n");
  fprintf(fp, "#define VCPU_EDX_OFF VCPU_REGS_OFF(2)\n");
  fprintf(fp, "#define VCPU_EBX_OFF VCPU_REGS_OFF(3)\n");
  fprintf(fp, "#define VCPU_ESP_OFF VCPU_REGS_OFF(4)\n");
  fprintf(fp, "#define VCPU_EBP_OFF VCPU_REGS_OFF(5)\n");
  fprintf(fp, "#define VCPU_ESI_OFF VCPU_REGS_OFF(6)\n");
  fprintf(fp, "#define VCPU_EDI_OFF VCPU_REGS_OFF(7)\n");
  fprintf(fp, "#define VCPU_FXSTATE_OFF %d\n", offsetof(vcpu_t, fxstate));
  fprintf(fp, "#define VCPU_SEGS_OFF(i) (%d+i*%d)\n",
      offsetof(vcpu_t, segs[0].selector), sizeof vcpu.segs[0]);
  fprintf(fp, "#define VCPU_SEGS_BASE_OFF(i) (%d+i*%d)\n",
      offsetof(vcpu_t, segs[0].base), sizeof vcpu.segs[0]);
  fprintf(fp, "#define VCPU_ORIG_SEGS_OFF(i) (%d+i*%d)\n",
      offsetof(vcpu_t, orig_segs[0]), sizeof vcpu.orig_segs[0]);
  fprintf(fp, "#define VCPU_SEGS_STRUCT_SIZE %d\n", sizeof vcpu.segs[0]);
  fprintf(fp, "#define VCPU_ES_OFF VCPU_SEGS_OFF(%d)\n", R_ES);
  fprintf(fp, "#define VCPU_FS_OFF VCPU_SEGS_OFF(%d)\n", R_FS);
  fprintf(fp, "#define VCPU_GS_OFF VCPU_SEGS_OFF(%d)\n", R_GS);
  fprintf(fp, "#define VCPU_CS_OFF VCPU_SEGS_OFF(%d)\n", R_CS);
  fprintf(fp, "#define VCPU_SS_OFF VCPU_SEGS_OFF(%d)\n", R_SS);
  fprintf(fp, "#define VCPU_DS_OFF VCPU_SEGS_OFF(%d)\n", R_DS);
  fprintf(fp, "#define VCPU_PREV_TB_OFF %d\n", offsetof(vcpu_t, prev_tb));
  fprintf(fp, "#define VCPU_EDGE_OFF %d\n", offsetof(vcpu_t, edge));
  fprintf(fp, "#define VCPU_N_EXEC_OFF %d\n", offsetof(vcpu_t, n_exec));
  fprintf(fp, "#define VCPU_REPLAY_LAST_ENTRY_N_EXEC_OFF %d\n",
			offsetof(vcpu_t, replay_last_entry_n_exec));
  fprintf(fp, "#define VCPU_REPLAY_LOG_OFF %d\n", offsetof(vcpu_t, replay_log));
  fprintf(fp, "#define VCPU_SCRATCH_OFF(i) (%d + i*%d)\n",
      offsetof(vcpu_t, scratch), sizeof vcpu.scratch[0]);
  fprintf(fp, "#define VCPU_JTARGET_OFF %d\n", offsetof(vcpu_t, jtarget));
  fprintf(fp, "#define VCPU_CALLOUT_ARGS_OFF %d\n",
      offsetof(vcpu_t, callout_args));
  fprintf(fp, "#define VCPU_CALLOUT_OFF %d\n", offsetof(vcpu_t, callout));
  fprintf(fp, "#define VCPU_CALLOUT_N_ARGS_OFF %d\n",
      offsetof(vcpu_t, callout_n_args));
  fprintf(fp, "#define VCPU_CALLOUT_NEXT_OFF %d\n",
      offsetof(vcpu_t, callout_next));
  fprintf(fp, "#define VCPU_CALLOUT_CUR_OFF %d\n",
      offsetof(vcpu_t, callout_cur));
  fprintf(fp, "#define VCPU_TC_PTR_OFF %d\n", offsetof(vcpu_t, tc_ptr));
  fprintf(fp, "#define VCPU_CALLOUT_LABEL_OFF %d\n",
      offsetof(vcpu_t, callout_label));
  fprintf(fp, "#define VCPU_FUNC_LABEL_OFF %d\n",
      offsetof(vcpu_t, func_label));
  fprintf(fp, "#define VCPU_FUNC_ARGS_OFF %d\n",
      offsetof(vcpu_t, cur_func_struct.func_args));
  fprintf(fp, "#define VCPU_FUNC_OFF %d\n",
			offsetof(vcpu_t, cur_func_struct.func));
  fprintf(fp, "#define VCPU_FUNC_N_ARGS_OFF %d\n",
      offsetof(vcpu_t, cur_func_struct.func_n_args));
  fprintf(fp, "#define VCPU_FUNC_STACK_OFF %d\n",
      offsetof(vcpu_t, cur_func_struct.func_stack));
  fprintf(fp, "#define VCPU_FUNC_MONITOR_STATE_OFF %d\n",
      offsetof(vcpu_t, cur_func_struct.func_monitor_state));
  fprintf(fp, "#define VCPU_FUNC_INTR_FRAME_STATE_OFF %d\n",
      offsetof(vcpu_t, func_intr_frame_monitor_state));
  fprintf(fp, "#define VCPU_FUNC_MONITOR_EIP_OFF %d\n",
      offsetof(vcpu_t, func_monitor_eip));
  fprintf(fp, "#define VCPU_DEFAULT_USER_GS_OFF %d\n",
      offsetof(vcpu_t, default_user_gs));

  fprintf(fp, "#ifndef JUMPTABLE1_MASK\n"
              "#define JUMPTABLE1_MASK %#x\n"
              "#endif\n", JUMPTABLE1_MASK);
  fprintf(fp, "#define VCPU_TEMPORARIES_OFF(i) (%d + i*%d)\n",
      offsetof(vcpu_t, temporaries), sizeof vcpu.temporaries[0]);

  fprintf(fp, "#define MONITOR_EFLAGS_OFF %d\n", offsetof(monitor_t, eflags));
	/*
  fprintf(fp, "#define MONITOR_REGS_OFF(i) (%d+%d*i)\n",
      offsetof(monitor_t, regs[0]), sizeof monitor.regs[0]);
  fprintf(fp, "#define MONITOR_EAX_OFF MONITOR_REGS_OFF(0)\n");
  fprintf(fp, "#define MONITOR_ECX_OFF MONITOR_REGS_OFF(1)\n");
  fprintf(fp, "#define MONITOR_EDX_OFF MONITOR_REGS_OFF(2)\n");
  fprintf(fp, "#define MONITOR_EBX_OFF MONITOR_REGS_OFF(3)\n");
  fprintf(fp, "#define MONITOR_ESP_OFF MONITOR_REGS_OFF(4)\n");
  fprintf(fp, "#define MONITOR_EBP_OFF MONITOR_REGS_OFF(5)\n");
  fprintf(fp, "#define MONITOR_ESI_OFF MONITOR_REGS_OFF(6)\n");
  fprintf(fp, "#define MONITOR_EDI_OFF MONITOR_REGS_OFF(7)\n");
			*/
  fprintf(fp, "#define MONITOR_EAX_OFF %d\n", offsetof(monitor_t, eax));
  fprintf(fp, "#define MONITOR_ECX_OFF %d\n", offsetof(monitor_t, ecx));
  fprintf(fp, "#define MONITOR_EDX_OFF %d\n", offsetof(monitor_t, edx));
  fprintf(fp, "#define MONITOR_EBX_OFF %d\n", offsetof(monitor_t, ebx));
  fprintf(fp, "#define MONITOR_ESP_OFF %d\n", offsetof(monitor_t, esp));
  fprintf(fp, "#define MONITOR_EBP_OFF %d\n", offsetof(monitor_t, ebp));
  fprintf(fp, "#define MONITOR_ESI_OFF %d\n", offsetof(monitor_t, esi));
  fprintf(fp, "#define MONITOR_EDI_OFF %d\n", offsetof(monitor_t, edi));
  fprintf(fp, "#define MONITOR_ES_OFF %d\n", offsetof(monitor_t, es));
  fprintf(fp, "#define MONITOR_SS_OFF %d\n", offsetof(monitor_t, ss));
  fprintf(fp, "#define MONITOR_DS_OFF %d\n", offsetof(monitor_t, ds));
  fprintf(fp, "#define MONITOR_FS_OFF %d\n", offsetof(monitor_t, fs));
  fprintf(fp, "#define MONITOR_EIP_OFF %d\n",
      offsetof(monitor_t, eip));
	/*
  fprintf(fp, "#define MONITOR_SEGS_OFF(i) (%d+%d*i)\n",
      offsetof(monitor_t, segs[0]), sizeof monitor.segs[0]);
  fprintf(fp, "#define MONITOR_ES_OFF MONITOR_SEGS_OFF(0)\n");
  fprintf(fp, "#define MONITOR_FS_OFF MONITOR_SEGS_OFF(1)\n");
  fprintf(fp, "#define MONITOR_GS_OFF MONITOR_SEGS_OFF(2)\n");
  fprintf(fp, "#define MONITOR_CS_OFF MONITOR_SEGS_OFF(3)\n");
  fprintf(fp, "#define MONITOR_SS_OFF MONITOR_SEGS_OFF(4)\n");
  fprintf(fp, "#define MONITOR_DS_OFF MONITOR_SEGS_OFF(5)\n");
	*/
  fprintf(fp, "#endif\n");
  fclose(fp);
}
    ...
    size_t codesize, int operand_size)
{
  ...
  while (ptr - code < (int)codesize) {
    ...
    /*
    char str[128];
    sprint_insn((unsigned long)ptr, str, sizeof str, operand_size, false);
    printf("disassembling insn: %s[%02hx %02hx %02hx %02hx %02hx]\n", str,
        *ptr, *(ptr+1), *(ptr+2), *(ptr+3), *(ptr+4));
        */
    len = disas_insn(ptr, 0, &insns[n_insns], operand_size, false);
    len = disas_insn(ptr, -len, &insns[n_insns], operand_size, false);
    if (len == 0) {
      return 0;
    }
    ptr += len;
  ...
  for (i = 0; i < assignments->num_assignments; i++) {
    ...
    if (!assignments_getval(&ordered_assignments, &assignments->arr[i].var,
      ...
      assignments_add(&ordered_assignments, &assignments->arr[i].var,
          &assignments->arr[i].val);
      /*
      temporaries[n_temporaries] = assignments->arr[i].var.tag.reg;
        n_temporaries++;
        */
    }
  }
...
{
  FILE *outfile, *vars_fp, *ordered_vars_fp;
  insn_t insns[MAX_PEEP_SIZE];
  EXE_SYM *sym;
  int i, n_insns = 0;
    ...
    struct nomatch_pair_t const *nomatch_pairs, int num_nomatch_pairs)
{
  ...
  for (i = 0; i < n_insns; i++) {
    ...
    int j;
    fprintf(outfile, "{%d, \"%s\",{",insns[i].opc,opctable_name(insns[i].opc));
    for (j = 0; j < 3; j++) {
  ...
  /*
  fprintf(outfile, "},%#llx,%#hhx,%d,{", *cpu_constraints,
      *used_regs, num_nomatch_pairs);
      */
...
{
  FILE *outfile, *vars_fp, *constraints_fp/*, *used_regs_fp*/;
  ...
  //used_regs_fp = xfopen("used_regs", "r");
  ...
  for (i = 0; i < num_entries - 1; i++) {
    ...
    //read_used_regs(used_regs_fp, name, &used_regs);
    ...
    int regno)
{
  ...
  for (i = 0; i < assignments->num_assignments; i++) {
    if (assignments->arr[i].var.type == op_reg) {
      if (regno-- == 0) {
        operand2str(&assignments->arr[i].var, regname, regname_size);
        ASSERT(regname[0] == '%');
        memmove(regname, regname + 1, strlen(regname));
        return;
      }
  ...
  ERR("Request for register number %d in assignments, but there are not that "
      "many registers. num_assignments=%d\n", regno,
    ...
    int segno)
{
  ...
  for (i = 0; i < assignments->num_assignments; i++) {
    if (assignments->arr[i].var.type == op_seg) {
      if (segno-- == 0) {
        operand2str(&assignments->arr[i].var, segname, segname_size);
        ASSERT(segname[0] == '%');
        memmove(segname, segname + 1, strlen(segname));
        return;
      }
  ...
  ERR("Request for segment number %d in assignments, but there are not that "
      "many segments.\n", segno);
  ASSERT(0);
  */
}
...
is_variable_name(char const *name, assignments_t const *assignments)
{
  ...
  if (!strcmp(name, "cur_addr") || !strcmp(name, "fallthrough_addr")) {
    return true;
  }
...
output_constant_variable_code(FILE *outfile, EXE_SYM **entries, int entrynum,
    assignments_t const *assignments, char const *optr)
{
  ...
  for (i = 0, rel = relocs; i < nb_relocs; i++, rel++) {
    ...
    if (offset >= val && offset < val + size) {
      ...
      if (get_reloc_expr(relname, sizeof relname, sym_name, assignments)) {
      ...
      switch (type) {
        ...
        case R_386_PC32:
          ...
          if (strstr(relname, "target_C0")) {
            fprintf(outfile, "  jmp_offset[0] = %d;\n", reloc_offset);
          }
          if (strstr(relname, "tc_next_eip")) {
            fprintf(outfile, "  jmp_offset[1] = %d;\n", reloc_offset);
          }
          break;
        default:
          ERR("Unsupported i386 relocation (%d) for %s\n", type, sym_name);
          exit(1);
      }
    }
  }
...
output_regbit_code(char *buf, size_t buf_size, uint8_t const *code[],
    size_t size, int regtype, int bitnum, bool set, char const *optr)
{
  uint8_t xor[size];
  int i, num_regbits;
  char *ptr = buf, *end = buf + buf_size;
  ...
  for (b = 0; b < size; b++) {
    ...
    //printf("Checking bit %d", b);
    if (code[0] && code[1 << i]) {
      code_set = code[1 << i];
      code_reset = code[0];
      //printf(" using 0 and %d\n", 1 << i);
    } else {
      for (j = 0; j < num_regbits; j++) {
        if (j != i && code[1 << j] && code[(1 << j) + (1 << i)]) {
          //printf("code_set:%d, reset:%d\n", (1 << j) + (1 << i), 1 << j);
      ...
      if (j == num_regbits) {
        printf("Code reset/set pair not found for bitnum %d.\n", i);
        exit(1);
      }
    }
    ...
    int regno, char const *optr)
{
  ...
  for (i = 0; i < num_regbits; i++) {
    if (output_regbit_code(buf, sizeof buf, code, size, regtype, i, true,
      ...
      output_regbit_code(buf, sizeof buf, code, size, regtype, i, false, optr);
      fprintf(outfile, "%s", buf);
      fprintf(outfile, "  }\n");
    }
  }
...
output_register_variable_code(FILE *outfile, EXE_SYM **entries, int entrynum,
    int num_variants, assignments_t const *assignments, char const *optr)
{
  EXE_SYM *sym = entries[entrynum], *next_sym = entries[entrynum + 1];
  Elf32_Word size = next_sym->st_value - sym->st_value;
  char const *name = get_sym_name(sym);
  ...
  for (i = entrynum + 1; i < entrynum + num_variants; i++) {
    ...
    //printf("Outputting code for %s:%d:%d\n", name, regtype, regno);
...
{
  int edgenum;
  edge_offsets[0] = edge_offsets[1] = 0xffff;
    ...
    char const *optr)
{
  ...
  output_register_variable_code(outfile, entries, i, num_variants,
      assignments, optr);
  output_constant_variable_code(outfile, entries, i, assignments, optr);
  fprintf(outfile, "  %s += %d;\n", optr, size);
  return num_variants;
}
    ...
    char const *filename, char const *prefix)
{
  int i;
  FILE *vars_fp;
  int num_variants, rb_num_variants;
  assignments_t assignments;
  int num_entries, num_rb_entries;
  uint16_t edges[2];
  ...
  EXE_SYM *entries[nb_syms];
  EXE_SYM *rb_entries[nb_syms];
  char rollback_prefix[strlen(prefix) + 64];
  snprintf(rollback_prefix, sizeof rollback_prefix, "%s%s",
      ROLLBACK_PREFIX_STR, prefix);

  num_entries = get_symtab_entries(entries, prefix);
  num_rb_entries = get_symtab_entries(rb_entries, rollback_prefix);
  ASSERT(num_entries == num_rb_entries);

  for (i = 0; i < num_entries - 1; i += num_variants) {
    EXE_SYM *sym = entries[i], *rb_sym = rb_entries[i];
    char const *name = get_sym_name(sym);
    EXE_SYM *next_sym = entries[i+1], *rb_next_sym = rb_entries[i+1];
    Elf32_Addr val = sym->st_value, rb_val = rb_sym->st_value;
    Elf32_Word size = next_sym->st_value - sym->st_value;
    Elf32_Word rb_size = rb_next_sym->st_value - rb_sym->st_value;
    Elf32_Addr excp_val;
    int nb_rollbacks;
    int j, s;
    read_assignments(vars_fp, name, &assignments);
    ...
    for (j = 0; j < assignments.num_assignments; j++) {
      ...
      if (opstr[0] == '%') {
        /* for registers, remove the '%' sign. */
        memmove(opstr, opstr + 1, strlen(opstr));
      }
      fprintf(outfile, "  %s = *peep_param_ptr++;\n", opstr);
    }

    /* edge_offsets, target_C0, tc_next_eip. */
    output_edge_offsets(outfile, sym, size, edges);
    if (edges[0] != 0xffff) {
      fprintf(outfile, "  target_C0 = (long)gen_code_ptr + "
          "edge_offset[0];\n");
    }
    if (edges[1] != 0xffff) {
      fprintf(outfile, "  tc_next_eip = (long)gen_code_ptr + "
          "edge_offset[1];\n");
    }

    /* If fallthrough_addr is not zero, this instruction must be a terminating
     * instruction. */
    fprintf(outfile, "  if (is_terminating) {\n");
    fprintf(outfile, "    tc_end = 0;\n");
    fprintf(outfile, "  } else {\n");
    fprintf(outfile, "    tc_end = (long)gen_code_ptr+%d;\n", size);
    fprintf(outfile, "  }\n");

    num_variants = output_gencode_for_entry(outfile, codefile, entries, i,
        prefix, &assignments, "gen_code_ptr");
    fprintf(outfile, "  if (!rollbacks) {\n");
    fprintf(outfile, "    break;\n");
    fprintf(outfile, "  }\n");
    rb_num_variants = output_gencode_for_entry(outfile, codefile, rb_entries, i,
        rollback_prefix, &assignments, "rollback_ptr");
    ASSERT(rb_num_variants == 0 || rb_num_variants == num_variants);
    /* Find all excp's and relate them in code_offset <-> rb_offset. */
    nb_rollbacks = 0;
    for (s = 0, sym = symtab; s < nb_syms; s++, sym++) {
      char const *name = get_sym_name(sym);
      Elf32_Addr excp_val = sym->st_value;
      if (   excp_val >= val && excp_val < val + size
          && strstart(name, EXCP_PATTERN, NULL)) {
        fprintf(outfile, "  if (rollbacks->code_offset) {\n");
        fprintf(outfile, "    rollbacks->code_offset[%d] = 0x%x;\n",
            nb_rollbacks, excp_val - val);
        fprintf(outfile, "  }\n");
        nb_rollbacks++;
      }
    }
    fprintf(outfile, "  rollbacks->nb_rollbacks = %d;\n", nb_rollbacks);
    nb_rollbacks--;
    for (s = 0, sym = symtab; s < nb_syms; s++, sym++) {
      char const *name = get_sym_name(sym);
      Elf32_Addr excp_val = sym->st_value;
      if (   excp_val >= rb_val && excp_val < rb_val + rb_size
          && strstart(name, ROLLBACK_PREFIX_STR EXCP_PATTERN, NULL)) {
        fprintf(outfile, "  if (rollbacks->rb_offset) {\n");
        fprintf(outfile, "    rollbacks->rb_offset[%d] = 0x%x;\n",
            nb_rollbacks, excp_val - rb_val);
        fprintf(outfile, "  }\n");
        nb_rollbacks--;
      }
    }
...
output_gencode(char const *outfilename)
{
  ...
  fprintf(outfile, "size_t\npeepgen_code(peepgen_label_t label, "
      "long *peep_param_buf, uint8_t *gen_code_buf, uint16_t *edge_offset, "
      "uint16_t *jmp_offset, rollbacks_t *rollbacks, long cur_addr, "
			"long fallthrough_addr, int is_terminating)\n");
  fprintf(outfile, "{\n");
  fprintf(outfile, "uint8_t *gen_code_ptr = gen_code_buf;\n");
  fprintf(outfile, "long *peep_param_ptr = peep_param_buf;\n");
  fprintf(outfile, "static uint16_t dummy[2];\n");
  fprintf(outfile, "uint8_t *rollback_ptr;\n\n");
  fprintf(outfile, "long vr0d=-1, vr1d=-1, vr2d=-1, vseg0=-1, vseg1=-1, "
			"tc_end=-1, C0, C1, target_C0, tc_next_eip, tr0d=-1, tr1d=-1, "
			"tr2d=-1, prefixNN;\n\n");
  //fprintf(outfile, "if (!dummy) { dummy = palloc_get_page(PAL_ASSERT); }\n");
  fprintf(outfile, "if (!jmp_offset) { jmp_offset = dummy; }\n");
  fprintf(outfile, "if (!edge_offset) { edge_offset = dummy; }\n");
  fprintf(outfile, "if (rollbacks) { rollback_ptr = rollbacks->buf; }\n");
  fprintf(outfile, "else { rollback_ptr = NULL; }\n");
  ...
  output_gencode_and_rollback_from_file(outfile, codefile, "out.o",
      PEEP_PREFIX_STR);
  /*
  output_gencode_from_file(outfile, codefile, "peepgen_snippets.o",
      SNIPPET_PREFIX_STR);
   */
  ...
  fprintf(outfile, "}\n");    /* switch. */
  fprintf(outfile, "if (rollbacks) {\n");
  fprintf(outfile, "  rollbacks->buf_size = rollback_ptr - "
      "(uint8_t *)rollbacks->buf;\n");
  fprintf(outfile, "}\n");
  fprintf(outfile, "the_end: return gen_code_ptr - gen_code_buf;\n");
  fprintf(outfile, "}\n");    /* function close. */
  fclose(codefile);
  fclose(outfile);
}
...
insns_are_isomorphic(struct insn_t const *a, struct insn_t const *b)
{
  ...
  for (i = 0; i < 3; i++) {
    ...
    /*
    if (a->op[i].rex_used != b->op[i].rex_used) {
      printf("rex_used not equal.\n");
      return false;
    }
    */
  }
    ...
    size_t n_insns, EXE_SYM **entries)
{
  ...
  /*
  printf("%s: renaming from constant %d to variable %d\n", name, regval,
      regnum);
      */
  ...
  //char buf[4096];
  //insns2str(insns, n_insns, buf, sizeof buf);
...
/* dummy functions. */
#if 0
bool read_segment(uint32_t *e1, uint32_t *e2, uint16_t selector, bool shadow) {
  return true;
}
...
struct thread *thread_initial(void) {
  return NULL;
}
