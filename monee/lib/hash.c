...
static struct hash_elem *find_elem (struct hash *, struct list *,
                                    struct hash_elem *);
static void
find_iterator (struct hash *h, struct list *bucket, struct hash_elem *e,
...
/* Returns a list of all items whose hash value is equal to the
   hash value of E. */
struct list *
    ...
    struct hash_iterator *i)
{
  ...
  i->hash = h;
  i->bucket = bucket;
  i->elem = list_elem_to_hash_elem (list_head(bucket));
  for (l = list_begin (bucket); l != list_end (bucket); l = list_next (l)) {
    struct hash_elem *hi = list_elem_to_hash_elem (l);
    if (h->equal (hi, e, h->aux)) {
      return;
    }
    i->elem = list_elem_to_hash_elem(l);
  }
}
