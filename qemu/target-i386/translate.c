...
static void gen_lea_modrm(DisasContext *s, int modrm, int *reg_ptr,
    int *offset_ptr, memaccess_t *memaccess)
{
    target_long disp;
    int havesib;
    int base;
    ...
    memaccess_t l_memaccess;

    if (!memaccess) {
      /* The caller passed memaccess==NULL. Set memaccess to a dummy value, to
       * avoid checking memaccess for NULL each time memaccess is accessed.
       */
      memaccess = &l_memaccess;
    }
    ...
    if (s->aflag) {
        ...
        if (havesib && (index != 4 || scale != 0)) {
            ...
            if (s->aflag == 2) {
                ...
                memaccess->valid = 1;
                memaccess->scale = scale;
                memaccess->index = index;
            } else 
#endif
            {
        ...
        case 0:
            ...
            gen_op_addl_A0_reg_sN[0][R_ESI]();
            memaccess->valid = 1;
            memaccess->base = R_EBX;
            memaccess->index = R_ESI;
            memaccess->scale = 0;
            break;
        case 1:
            gen_op_movl_A0_reg[R_EBX]();
        ...
        case 4:
            gen_op_movl_A0_reg[R_ESI]();
            memaccess->valid = 1;
            memaccess->base = R_ESI;
            break;
        case 5:
            gen_op_movl_A0_reg[R_EDI]();
            memaccess->valid = 1;
            memaccess->base = R_EDI;
            break;
        case 6:
            gen_op_movl_A0_reg[R_EBP]();
            memaccess->valid = 1;
...
/* generate modrm memory load or store of 'reg'. TMP0 is used if reg !=
   OR_TMP0 */
static void gen_ldst_modrm(DisasContext *s, int modrm, int ot, int reg,
    int is_store, memaccess_t *memaccess)
{
  ...
  if (mod == 3) {
    ...
    gen_lea_modrm(s, modrm, &opreg, &disp, memaccess);
    if (is_store) {
      if (reg != OR_TMP0)
        gen_op_mov_TN_reg[ot][0][reg]();
      gen_op_st_T0_A0[ot + s->mem_index]();
    ...
    memaccess_t *memaccess)
{
    ...
    if (sse_op2 == SSE_SPECIAL) {
        ...
        case 0x0e7: /* movntq */
            if (mod == 3) 
                goto illegal_op;
            gen_lea_modrm(s, modrm, &reg_addr, &offset_addr, memaccess);
...
#ifdef TARGET_X86_64
            if (s->dflag == 2) {
                gen_ldst_modrm(s, modrm, OT_QUAD, OR_TMP0, 0, &s->memdef);
                gen_op_movq_mm_T0_mmx(offsetof(CPUX86State,fpregs[reg].mmx));
            } else 
#endif
            {
                gen_ldst_modrm(s, modrm, OT_LONG, OR_TMP0, 0, &s->memdef);
        ...
        case 0x7e: /* movd ea, mm */
            ...
            if (s->dflag == 2) {
                ...
                gen_ldst_modrm(s, modrm, OT_QUAD, OR_TMP0, 1, &s->memdef);
            } else 
#endif
            {
                gen_op_movl_T0_mm_mmx(offsetof(CPUX86State,fpregs[reg].mmx));
                gen_ldst_modrm(s, modrm, OT_LONG, OR_TMP0, 1, &s->memdef);
            }
            break;
        ...
        case 0x32a: /* cvtsi2sd */
            ...
            gen_ldst_modrm(s, modrm, ot, OR_TMP0, 0, &s->memdef);
            op1_offset = offsetof(CPUX86State,xmm_regs[reg]);
        ...
        case 0x1c4: 
            s->rip_offset = 1;
            gen_ldst_modrm(s, modrm, OT_WORD, OR_TMP0, 0, &s->memdef);
            val = ldub_code(s->pc++);
            if (b1) {
                val &= 7;
 ...
 reswitch:
    ...
    case 0x83:
        {
            ...
            if (mod != 3) {
                ...
                gen_lea_modrm(s, modrm, &reg_addr, &offset_addr, &s->memdef);
                opreg = OR_TMP0;
            } else {
                opreg = rm;
            }
    ...
    case 0x8f: /* pop Ev */
        ...
        if (mod == 3) {
            ...
            gen_ldst_modrm(s, modrm, ot, OR_TMP0, 1, &s->memdef);
            s->popl_esp_hack = 0;
            gen_pop_update(s);
        }
        break;
    ...
    case 0x89: /* mov Gv, Ev */
        ...
        gen_ldst_modrm(s, modrm, ot, reg, 1, &s->memuse);
        break;
    case 0xc6:
    case 0xc7: /* mov Ev, Iv */
        if ((b & 1) == 0)
    ...
    case 0x8e: /* mov seg, Gv */
        ...
        if (reg >= 6 || reg == R_CS)
            goto illegal_op;
        gen_ldst_modrm(s, modrm, OT_WORD, OR_TMP0, 0, &s->memuse);
        gen_movl_seg_T0(s, reg, pc_start - s->cs_base);
        if (reg == R_SS) {
            /* if reg == SS, inhibit interrupts/trace */
            /* If several instructions disable interrupts, only the
    ...
    case 0x8c: /* mov Gv, seg */
        ...
        if (mod == 3)
            ...
            ot = OT_WORD;
        gen_ldst_modrm(s, modrm, ot, OR_TMP0, 1, &s->memuse);
        break;
    ...
    case 0x1bf: /* movswS Gv, Eb */
        {
            ...
            if (mod == 3) {
                ...
                gen_lea_modrm(s, modrm, &reg_addr, &offset_addr, &s->memuse);
                if (b & 8) {
    ...
    case 0x8d: /* lea */
        ...
        val = s->addseg;
        s->addseg = 0;
        gen_lea_modrm(s, modrm, &reg_addr, &offset_addr, NULL);
        s->addseg = val;
    ...
    case 0x190 ... 0x19f: /* setcc Gv */
        modrm = ldub_code(s->pc++);
        gen_setcc(s, b);
        gen_ldst_modrm(s, modrm, OT_BYTE, OR_TMP0, 1, &s->memdef);
        break;
    ...
    case 0x101:
        ...
        case 4: /* smsw */
            ...
            gen_ldst_modrm(s, modrm, OT_WORD, OR_TMP0, 1, &s->memdef);
            break;
        case 6: /* lmsw */
            if (s->cpl != 0) {
                gen_exception(s, EXCP0D_GPF, pc_start - s->cs_base);
    ...
    case 0x103: /* lsl */
        ...
        gen_op_mov_reg_T1[ot][reg]();
        break;
/* mdbg functions : sorav : begin */
    case 0x104:
        modrm = ldub_code(s->pc++);
        val = ldl_code(s->pc);
        s->pc += 4;
        switch(modrm) {
        case 0x00: /* print machine state*/
          gen_op_print_state(s->pc - 7);
          break;
        case 0x01:
          gen_op_inspect_memory(val & 0xffffffff, 0);
          break;
        case 0x02:
          gen_op_set_mem_monitor(val & 0xffffffff, 0);
          break;
        case 0x03:
          gen_op_reset_mem_monitor(val & 0xffffffff, 0);
          break;
        case 0x04: /* print error message */
          gen_op_print_errormsg();
          break;
        case 0x05:
          gen_op_profile_reset();
          break;
        case 0x06: /* set */
          gen_op_read_mdbg_level(val);
          break;
        case 0x07:
          gen_op_romwrite_set_dst(val);
          break;
        case 0x08:
          gen_op_romwrite_set_src(val);
          break;
        case 0x09:
          gen_op_romwrite_set_size(val);
          break;
        case 0x0a:
          gen_op_romwrite_transfer();
          break;
        default:
          goto illegal_op;
        }
        break;
/* mdbg_functions : sorav : end */
    case 0x118:
        modrm = ldub_code(s->pc++);
        mod = (modrm >> 6) & 3;
        op = (modrm >> 3) & 7;
    ...
    case 0x1c3: /* MOVNTI reg, mem */
        ...
        reg = ((modrm >> 3) & 7) | rex_r;
        /* generate a generic store */
        gen_ldst_modrm(s, modrm, ot, reg, 1, &s->memdef);
        break;
    case 0x1ae:
        modrm = ldub_code(s->pc++);
        mod = (modrm >> 6) & 3;
    ...
    case 0x1c4 ... 0x1c6:
    case 0x1d0 ... 0x1fe:
        gen_sse(s, b, pc_start, rex_r, &s->memdef);
        break;
    default:
        goto illegal_op;
    }
...
static void
gen_print_memaccess(DisasContext *dc)
{
  if (dc->memdef.valid) {
    gen_op_print_memaccess(((dc->memdef.seg & 0x7) << 15)
        | ((dc->memdef.base & 0x1f) << 10)
        | ((dc->memdef.scale & 0x7) << 5)
        | (dc->memdef.index & 0x1f),
        dc->memdef.disp >> 32, dc->memdef.disp);
  }
}
...
/* sorav */
       if (rr_log) {
         gen_op_check_breakpoint(pc_ptr - dc->cs_base);
       }
       if (profile_log) {
         gen_op_count_executions(pc_ptr - dc->cs_base);
       }
/* sorav */
...
/*sorav*/
       gen_update_cc_op(dc);
       //gen_op_temporary_check(pc_ptr);
       switch (mdbg_level) {
         case 0:
           break;
         case 1:
           gen_op_check_stack_pointers();
           break;
         case 8:
           {
             static target_ulong prev_pc_ptr = 0;
             if (prev_pc_ptr != pc_ptr) {
               gen_op_print_state(pc_ptr);
               prev_pc_ptr = pc_ptr;
             }
           }
           break;
         case 9:
           gen_op_print_eip(pc_ptr);
           gen_print_memaccess(dc);
           break;
         case 10:
           gen_op_print_state(pc_ptr);
           gen_print_memaccess(dc);
           break;
         default:
           printf("Invalid mdbg_level (%d).\n", mdbg_level);
           exit(1);
       }

       /* memory monitor. */
       if (num_memory_monitors > 0) {
         gen_op_memory_monitor(pc_ptr);
       }
...
/*sorav*/
    ...
    *gen_opc_ptr = INDEX_op_end;
    ...
    /* optimize flag computations */
		/* sorav: flag optimizations do not play well with exceptions. diable them.
