                                  ...
                                  int index, int length, uint8_t *data)
{
    ...
    /* printf("%s() %d: request = %x, value=%x, DeviceRequest=%x, "
        "DeviceOutRequest=%x\n", __func__, __LINE__, request, value,
        DeviceRequest, DeviceOutRequest); */
    switch (request) {
...
USBDevice *usb_msd_init(const char *filename)
{
    ...
    bdrv = bdrv_new("usb");
		...
		flags |= strstr(filename, ".wr.fifo")?BDRV_O_APPEND|BDRV_O_LOG:0;
		flags |= strstr(filename, ".rd.fifo")?BDRV_O_RDONLY|BDRV_O_LOG:0;
    ...
    snprintf(s->dev.devname, sizeof(s->dev.devname), "QEMU:%.16s", filename);
